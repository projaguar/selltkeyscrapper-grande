...
1. 그룹 내(=이 앱이 만든) 기존 프로필 재사용
2. 부족하면 데스크탑 지문으로 신규 생성 (기본 2개)
3. 블록 감지 시 삭제 → 그룹 내 재생성

> ⚠️ **기존/미분류 프로필은 절대 건드리지 않는다**(흡수·이동 없음). 오직 이 전용 그룹 안에서만
> 생성·삭제한다. 덕분에 같은 머신·같은 AdsPower 계정을 쓰는 다른 앱(예: `prowler` 그룹)의
